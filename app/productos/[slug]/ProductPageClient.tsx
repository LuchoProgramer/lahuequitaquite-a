...
export default function ProductPageClient({ slug, initialProduct }: ProductPageClientProps) {
    ...
    React.useEffect(() => {
        // Si ya tenemos producto inicial y no hay sucursal seleccionada, no refetch
    ...
    const jsonLd = product ? {
        ...
        "offers": {
            "@type": "Offer",
            "url": `https://lahuequitaquitena.com/productos/${product.slug}`,
            "priceCurrency": "USD",
                ...
                <div className="mb-12">
                    ...
                    <div className="flex overflow-x-auto gap-4 pb-6 no-scrollbar snap-x">
                        {suggestions.length > 0 ? suggestions.map(item => (
                            <Link href={`/productos/${item.slug}`} key={item.id} className="snap-center shrink-0 w-[180px] flex flex-col gap-3 group cursor-pointer">
